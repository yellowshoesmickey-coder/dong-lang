...
io.on('connection', (socket) => {
  ...
  socket.on('join_by_id', (data) => {
    ...
    // Check game state — reject if game in progress (not lobby and not gameover)
    if (room.gameState && room.gameState !== 'lobby' && room.gameState !== 'gameover') {
      socket.emit('join_error', { message: 'Trò chơi đang diễn ra, không thể tham gia giữa chừng.' });
      return;
    }
  ...
  socket.on('get_active_rooms', () => {
    ...
    socket.emit('active_rooms', active);
  });
  ...
  // ── Player: Toggle agree to roles ──
  socket.on('agree_roles', (data) => {
    for (const [rid, room] of Object.entries(rooms)) {
      if (room.sockets.has(socket.id)) {
        const player = room.sockets.get(socket.id);
        player.agreed = data.agreed !== undefined ? data.agreed : !player.agreed;
        touchRoom(rid);
        io.to(rid).emit('room_updated', getRoomStatus(rid));
        checkAllAgreed(rid);
        break;
  ...
  socket.on('start_game', () => {
    for (const [rid, room] of Object.entries(rooms)) {
      if (room.sockets.has(socket.id) && room.sockets.get(socket.id).isHost) {
        ...
        // Check all agreed (excluding host — host doesn't need to agree)
        let allAgreed = true;
        for (const [sid, player] of room.sockets) {
          if (sid === room.host) continue;
