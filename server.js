...
const MULTI_ROLE_IDS = ['villager', 'werewolf'];
  ...
  socket.on('select_roles', (data) => {
    for (const [rid, room] of Object.entries(rooms)) {
      if (room.sockets.has(socket.id) && room.sockets.get(socket.id).isHost) {
        ...
        const numPlayers = room.sockets.size;
    ...
    for (const [rid, room] of Object.entries(rooms)) {
      if (room.sockets.has(socket.id) && room.sockets.get(socket.id).isHost) {
        ...
        if (!room.selectedRoles || room.selectedRoles.length === 0) {
          socket.emit('start_error', { message: 'No roles selected.' });
          return;
        }
        ...
        // Send role reveal to each player (only their own role)
        for (const [sid, player] of room.sockets) {
          const roleId = room.assignments[sid];
          const role = ROLES[roleId];
          const team = role.team;

          // If drunk, assign a random real role
          let realRoleId = roleId;
          if (role.drunkRole) {
            const nonFiller = room.selectedRoles.filter(r => !MULTI_ROLE_IDS.includes(r) && r !== 'drunk');
            if (nonFiller.length > 0) {
              realRoleId = nonFiller[Math.floor(Math.random() * nonFiller.length)];
            }
            // Store both: player sees "Drunk" but server knows real role
            player.drunkRealRole = realRoleId;
          }

          player.role = roleId;
          player.team = team;
          player.realRole = realRoleId || roleId;
          ...
          const revealData = {
            ...
            team: team,
            desc: role.desc,
            isDrunk: role.drunkRole,
            drunkRealRole: role.drunkRole ? ROLES[realRoleId]?.name : null,
            hasNightAction: !!role.action,
            action: role.action,
            isWolf: role.isWolf || false,
            isVampire: role.isVampire || false,
            canTalkToWolves: role.canTalkToWolves || false,
            knowsWolves: role.knowsWolves || false,
            winsWithWolves: role.winsWithWolves || false,
            cannotVote: role.cannotVote || false,
            winsIfHanged: role.winsIfHanged || false,
            revengeKill: role.revengeKill || false,
            immuneToWolf: role.immuneToWolf || false,
            switchesTeam: role.switchesTeam || false,
          };

          io.to(sid).emit('role_reveal', revealData);
        }
